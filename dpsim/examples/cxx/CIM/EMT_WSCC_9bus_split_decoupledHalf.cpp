...
void decoupleLine(SystemTopology &sys, const String &lineName, const String &node1, const String node2, String** dLineNames) {
  ...
  auto lineA = Signal::HalfDecouplingLineEMT::make(
    nameDLineA,
    Logger::Level::debug
  );

  auto lineB = Signal::HalfDecouplingLineEMT::make(
    nameDLineB,
    Logger::Level::debug
  );
  ...
  lineA->setParameters(
    ...
    Rline_scalar, Lline_scalar, Cline_scalar,lineB);
  sys.addComponent(lineA);
  sys.addComponents(lineA->getLineComponents());

  lineB->setParameters(
    sys.node<EMT::SimNode>(node2), sys.node<EMT::SimNode>(node1),
    Rline_scalar, Lline_scalar, Cline_scalar, lineA);
  sys.addComponent(lineB);
  sys.addComponents(lineB->getLineComponents());
}
